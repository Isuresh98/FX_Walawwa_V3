...
public class GameControll : MonoBehaviour {
    ...
    private void Start()
    {
       // Application.targetFrameRate = 60;  // Set frame rate to 60 FPS
        ...
        //player light up setup
        lightUp = FindObjectOfType<LightIntensityController>();
        // Setup slider
        if (timerSlider != null)
        {
            timerSlider.maxValue = lightUp.duration;
            timerSlider.value = lightUp.duration;
            timerSlider.value = 0;
        }

        // Set initial slider color to green
        if (sliderFill != null)
        {
    ...
    [Header("Player Light Up setting")]
    [SerializeField] float LightTimeDuration = 10f;
    LightIntensityController lightUp;
    [Header("UI Light Up setting Components")]
    public Slider timerSlider;        // Reference to the Slider UI
    public Image sliderFill;          // Reference to the Slider's Fill image
    ...
    public void SetLightUp()
    {
        ...
        if (m_eyesCount > 0&& !lightUp.isDecreasing)
            {
            
            lightUp.duration = LightTimeDuration;
                lightUp.StartDecrease();
                m_eyesCount -= 1;
    ...
    private void LightUpUISliderUpdate()
    {
        // Update slider value
        if (timerSlider != null)
        {
            timerSlider.value = lightUp.duration - lightUp.timer;
        }

        // Change slider color from green to red
        if (sliderFill != null)
        {
            sliderFill.color = Color.Lerp(Color.green, Color.red, lightUp.timer / lightUp.duration);
        }
    }
    ...
    public void GameOver()
    {
        ...
        GameObject[] interactObjects = GameObject.FindGameObjectsWithTag("Enemy");

        // Loop through each object and add the Outline component
        foreach (GameObject enemy in interactObjects)
        {
            // Get the EnemyAI component
            EnemyAI enemyAI = enemy.GetComponent<EnemyAI>();

            // Check if the component exists before calling StopFallow()
            if (enemyAI != null)
            {
                enemyAI.StopFallow();
            }
